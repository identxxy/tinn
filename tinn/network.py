...
class Network:
    ...
    def all_inference(self, input_vf: ti.template(), output_vf: ti.template()):
        ...
        if self.n_hidden_layers == 0:
            self._all_forward_last_layer(input_vf, output_vf, self.output_layer_w, self.output_layer_b)
        else:
            self._all_forward_one_layer(input_vf, self.mid_vf_eval, self.input_layer_w, self.input_layer_b)
            for i in range(self.n_hidden_layers - 1):
                self._all_forward_one_layer(self.mid_vf_eval, self.mid_vf_eval, self.hidden_layers_w_l[i], self.hidden_layers_b_l[i])
            self._all_forward_last_layer(self.mid_vf_eval, output_vf, self.output_layer_w, self.output_layer_b)
    ...
    def _all_train(self, input_vf: ti.template(), output_vf: ti.template()):
        ...
        if self.n_hidden_layers == 0:
            ...
            self._all_forward_one_layer(input_vf, self.mid_vf_train, self.input_layer_w, self.input_layer_b)
            for i in range(self.n_hidden_layers - 1):
                self._all_forward_one_layer(self.mid_vf_train, self.mid_vf_train, self.hidden_layers_w_l[i], self.hidden_layers_b_l[i])
            self._all_forward_last_layer(self.mid_vf_train, output_vf, self.output_layer_w, self.output_layer_b)

    @ti.kernel
    def _all_forward_one_layer(self,
    ...
    @ti.kernel
    def _all_forward_last_layer(self,
            input_vf: ti.template(),
            output_vf: ti.template(),
            weight_layer: ti.template(),
            bias_layer: ti.template()
